...
- 进入独立历史页分页浏览全部采集记录，并按设备类型、目标IP和有效状态筛选；
...
首页只展示最近 9 条采集记录；点击“查看全部历史”进入 `/history`，可以分页
浏览 `outputs/` 中的全部有效采集结果。历史筛选支持设备类型、完整或部分目标
IP/主机名、分析后有效状态以及每页数量。连接检查只验证能力且不会生成
`collection_summary.json`，因此不会进入历史采集记录。
历史列表会在 `outputs/.history_index.json` 保存可自动重建的派生索引，状态筛选
无需反复分析全部旧任务。索引使用文件元数据和固定上限的内容采样识别输入变化，
不会反复读取完整大日志；该索引不包含凭据，也不会修改原始采集总结。
...
- `GET /api/history`：历史记录分页与设备类型、目标IP、有效状态筛选；
